...
The proof is **complete** (the `phino` 0.0.138 normalization rules are confluent on
...
It supersedes the earlier minimal/extended development in this repository's history, which
proved confluence only for the minimal calculus and never finished the EO-matching variant.
...
| `phino` | `objectionary/phino` | The **authoritative, executable** rule spec. Rules live in `resources/normalize/*.yaml`. |
...
| This project | the present repo (`objectionary/proof`) | The new Lean 4 proof. |
...
The reduction relation `⟶` is the compatible (congruence) closure of the `phino` 0.0.138
rules: `dot, copy, alpha, overa, amiss, stay, over, stop, null, miss, dl, dd, dc, dca`, over `Term`
(formations `⟦B⟧`, applications `e(τ↦e')`, dispatches `e.τ`, the locators `Φ`/`ξ`, and the
terminator `⊥`). phino's fifteenth rule, `dotg`, fires only on the whole-program universe, which
`phino rewrite` never sees, so it is not modelled. The full pattern→result table, side conditions, and the eight deviations
...
* **Rule transcription vs phino.** The fifteen-rule *display* table (`Rules.lean`) is
  **generated** from pinned phino's `resources/normalize/*.yaml` by `gen-rules.py` before every build
...
* **Differential testing against phino.** `Difftest.lean` + `.github/difftest.sh` normalize
  ...
  **26/26**, exercising every modelled rule (incl. the positional rules' domain-ordinal skip of
  `Δ`/`λ` assets and `ρ`) on `⊥`-collapse *and* real formation results. This is the same `phino rewrite` mechanism the
...
Before investing in the Lean diamond, the central open question — *does the full
...
* **No genuine root critical pairs.** The two LHS-overlap groups are mutually exclusive by
  guards along root-stable axes — dispatch `⟦B⟧.τ` {dot, null, stop, dd} by slot-state
  (attached/void/absent) × `φ`-membership × subject-head; formation-app `⟦B⟧(τ↦e)`
  {copy, alpha, overa, amiss, over, stay, miss, dc} by slot-state × name-kind (positional `αᵢ` →
  alpha/overa/amiss by the state at its domain ordinal, named → copy/over/miss, `ρ` → stay) ×
  subject-head. The one overlap added since, `dl` on a formation holding both `λ` and `Δ`, joins
  at `⊥` because `dot` is guarded against such a formation (phino #1395). This rests on three
...
before every build by `.github/regen-rules.sh` from the `resources/normalize/*.yaml` of the phino
...
including the cases that diverged before `canon` (`⟦⟧(ρ↦Φ)`, bare value formations).
...
* **`WF`-scoped, by design.** The headline carries `WF e` = `legalKey` ∧ `Nodup`. Only `legalKey`
  ...
  *faithfulness* clause — matches Def. Binding, makes first-match `lookup` agree with phino's matcher
  ...
  now agree.)
* **`λ`/`Δ` atoms are outside the normalization relation `⟶`.** Only `dl` looks at them, to
  collapse a formation holding both to `⊥`. Their reduction is the paper's *separate* **Morphing** (`fig:morphing` — `Mlambda`
  ...
  fire on their own. This is the reason the paper's Appendix-A examples are validated through `difftest`
