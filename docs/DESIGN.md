...
* **Rule transcription vs phino.** The eleven-rule *display* table (`Rules.lean`) is
  **generated** from phino's `resources/*.yaml` by `gen-rules.py`; the CI build regenerates
  it from pinned phino and fails on any difference, so it cannot drift from phino. The
...
* **build** — install elan and `lake exe cache get`; run the generator unit tests
  (`scripts/tests`); regenerate `Rules.lean` and `RuleData.lean` from the pinned phino
  (`scripts/regen-rules.sh`) and fail if they differ from the committed files; `lake
...
* **difftest** — install the pinned phino binary (`.phino-version`, verified against
  `.phino-sha256`), build `difftest`, run `scripts/difftest.sh`; fails if our reducer
